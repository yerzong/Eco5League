/**
 * Campo de hora de formulario (estilo glass). Editable con teclado numérico
 * (máscara HH:MM) y un ícono de reloj que abre un DateTimePicker en
 * un bottom sheet animado — mismo patrón que FormDate.
 */
...
function timeToDate(timeStr: string): Date {
  ...
  if (parts.length >= 1 && !isNaN(parts[0])) { d.setHours(parts[0]); }
  if (parts.length >= 2 && !isNaN(parts[1])) { d.setMinutes(parts[1]); }
  return d;
}
...
export function FormTime({ value, onChange, placeholder = 'HH:MM', label, style }: FormTimeProps) {
  const [open, setOpen] = useState(false);
  const [temp, setTemp] = useState(() => timeToDate(value ?? ''));
  const translateY = useRef(new Animated.Value(400)).current;
  const scrimAnim = useRef(new Animated.Value(0)).current;

  const openSheet = () => {
    setTemp(timeToDate(value ?? ''));
    setOpen(true);
    Animated.parallel([
  ...
  const confirm = () => closeSheet(() => onChange?.(dateToTimeString(temp)));
    ...
    <View style={style}>
      <View style={styles.box}>
        ...
        <Pressable onPress={openSheet} hitSlop={12}>
          <IconClock size={18} color="rgba(246,246,248,0.55)" strokeWidth={2} />
        </Pressable>
      </View>
